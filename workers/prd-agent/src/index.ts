import { PRD_WORKER_PROMPT } from './systemPrompts';
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      context = body.context || { productSeed: body.productSeed || 'No product seed provided' };
    } catch (e) {
      context = { productSeed: 'No product seed provided' };
    }

    // Build complete prompt
    const fullPrompt = buildPrompt(PRD_WORKER_PROMPT, context);
    ...
    if (isStream) {
      ...
      ctx.waitUntil((async () => {
        try {
          // Send initial status
          await writer.write(encoder.encode(`data: ${JSON.stringify({ type: 'status', message: 'Analyzing product requirements...' })}\n\n`));
    ...
    try {
      ...
      return new Response(JSON.stringify({
        ...
        artifact: {
          type: 'markdown',
          content: '',
          logic_summary: 'Error occurred during PRD generation'
        },
        trace: {
          agent: 'prd-agent',
