import { DATA_WORKER_PROMPT } from './systemPrompts';
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const fullPrompt = buildPrompt(DATA_WORKER_PROMPT, context);
    ...
    if (isStream) {
      ...
      ctx.waitUntil((async () => {
        try {
          // Send initial status
          await writer.write(encoder.encode(`data: ${JSON.stringify({ type: 'status', message: 'Analyzing data requirements...' })}\n\n`));
    ...
    try {
      ...
      return new Response(JSON.stringify({
        system_state: {
          ...
          message: `Failed to generate data model: ${error.message}`
        },
        artifact: {
          type: 'code',
          content: '',
          logic_summary: 'Error occurred during data model generation'
        },
        trace: {
          agent: 'data-agent',
