import { API_WORKER_PROMPT } from './systemPrompts';
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const fullPrompt = buildPrompt(API_WORKER_PROMPT, context);
    ...
    if (isStream) {
      ...
      ctx.waitUntil((async () => {
        try {
          await writer.write(encoder.encode(`data: ${JSON.stringify({ type: 'status', message: 'Designing API architecture...' })}\n\n`));
    ...
    try {
      ...
      return new Response(JSON.stringify({
        system_state: {
          ...
          message: `Failed to generate API specification: ${error.message}`
        },
        artifact: {
          type: 'markdown',
          content: '',
          logic_summary: 'Error occurred during API specification generation'
        },
        trace: {
          agent: 'api-agent',
