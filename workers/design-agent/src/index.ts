import { DESIGN_WORKER_PROMPT } from './systemPrompts';
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const fullPrompt = buildPrompt(DESIGN_WORKER_PROMPT, context);
    ...
    if (isStream) {
      ...
      ctx.waitUntil((async () => {
        try {
          await writer.write(encoder.encode(`data: ${JSON.stringify({ type: 'status', message: 'Designing UI system...' })}\n\n`));
    ...
    try {
      ...
      return new Response(JSON.stringify({
        system_state: {
          ...
          message: `Failed to generate design system: ${error.message}`
        },
        artifact: {
          type: 'markdown',
          content: '',
          logic_summary: 'Error occurred during design generation'
        },
        trace: {
          agent: 'design-agent',
