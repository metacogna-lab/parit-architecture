...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const fullPrompt = buildPrompt(LOGIC_WORKER_PROMPT, context);
    ...
    if (isStream) {
      ...
      ctx.waitUntil((async () => {
        try {
          await writer.write(encoder.encode(`data: ${JSON.stringify({ type: 'status', message: 'Analyzing business logic...' })}\n\n`));
    ...
    try {
      ...
      return new Response(JSON.stringify({
        system_state: {
          ...
          message: `Failed to generate business logic: ${error.message}`
        },
        artifact: {
          type: 'markdown',
          content: '',
          logic_summary: 'Error occurred during business logic generation'
        },
        trace: {
          agent: 'logic-agent',
