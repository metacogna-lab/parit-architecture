import { DEPLOYMENT_WORKER_PROMPT } from './systemPrompts';
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const fullPrompt = buildPrompt(DEPLOYMENT_WORKER_PROMPT, context);
    ...
    if (isStream) {
      ...
      ctx.waitUntil((async () => {
        try {
          await writer.write(encoder.encode(`data: ${JSON.stringify({ type: 'status', message: 'Designing deployment strategy...' })}\n\n`));
    ...
    try {
      ...
      return new Response(JSON.stringify({
        system_state: {
          ...
          message: `Failed to generate deployment configuration: ${error.message}`
        },
        artifact: {
          type: 'code',
          content: '',
          logic_summary: 'Error occurred during deployment configuration generation'
        },
        trace: {
