...
export const PaperProgressDashboard: React.FC<PaperProgressDashboardProps> = ({ progress, stats }) => (<div className="fixed top-0 left-0 w-full z-[120] pointer-events-none p-6 flex justify-center"><div className="bg-white/90 backdrop-blur border-2 border-ink shadow-sm px-6 py-3 rounded-full pointer-events-auto flex items-center gap-6"><div className="flex items-center gap-3"><span className="font-serif font-bold text-lg">{progress}%</span><div className="w-24 h-2 bg-zinc-200 rounded-full overflow-hidden"><div className="h-full bg-emerald-500 transition-all duration-500" style={{ width: `${progress}%` }} /></div></div><div className="h-6 w-px bg-zinc-300" /><div className="flex gap-4">{stats.map((s, i) => <div key={i} className="flex flex-col items-center"><span className="text-[10px] uppercase font-bold text-zinc-400">{s.label}</span><div className={cn("w-2 h-2 rounded-full mt-1", s.status === 'complete' ? "bg-emerald-500" : s.status === 'active' ? "bg-amber-500 animate-pulse" : "bg-zinc-300")} /></div>)}</div></div></div>);
...
export const PaperThinkingOverlay: React.FC<PaperThinkingOverlayProps> = ({ step, agent }) => {
    return (
        <AnimatePresence mode="wait">
            {step && (
                <motion.div
                    ...
                    className="fixed bottom-36 left-1/2 -translate-x-1/2 z-[110] bg-ink text-white pl-4 pr-8 py-3 rounded-full shadow-hard flex items-center gap-4 border-2 border-white max-w-md w-full"
                >
                    <div className="relative shrink-0">
                        ...
                        <div className="absolute -top-1 -right-1 w-3 h-3 bg-accent rounded-full animate-ping border border-white" />
                    </div>
                    <div className="flex flex-col min-w-0 flex-1">
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-[10px] font-bold uppercase text-accent tracking-widest">{agent || "System"}</span>
                            <span className="text-[10px] font-mono text-zinc-500 animate-pulse">Thinking...</span>
                        </div>
                        <motion.span 
                            key={step} // Trigger animation on change
                            initial={{ opacity: 0, y: 5 }}
                            animate={{ opacity: 1, y: 0 }}
                            className="font-mono text-sm font-bold truncate text-zinc-100"
                        >
                            {step}
                        </motion.span>
                        {/* Fake progress bar for activity */}
...
// --- 4. Changelog ---
export const PaperChangelog: React.FC<PaperChangelogProps> = ({ commits }) => {
    return (
        <div className="p-8 h-full flex flex-col">
            <h2 className="font-serif font-black text-4xl mb-6">Audit Log</h2>
            
            <div className="flex-1 bg-white border-2 border-ink rounded-lg shadow-hard p-6 overflow-y-auto">
                <div className="relative border-l-2 border-zinc-200 ml-4 space-y-8 pl-8 py-2">
                    {commits.map((commit, i) => (
                        <div key={commit.id} className="relative group">
                            {/* Node on Timeline */}
                            <div className="absolute -left-[41px] top-1 w-6 h-6 bg-white border-2 border-ink rounded-full flex items-center justify-center z-10 group-hover:scale-110 transition-transform">
                                <GitBranch size={12} className="text-zinc-500" />
                            </div>
                            
                            <div className="flex flex-col gap-1">
                                <div className="flex items-center gap-3">
                                    <span className="font-mono text-xs font-bold bg-zinc-100 px-2 py-0.5 rounded text-zinc-500">{commit.hash}</span>
                                    <span className="text-xs text-zinc-400 font-mono">{commit.timestamp}</span>
                                </div>
                                <h4 className="font-bold text-lg">{commit.message}</h4>
                                <div className="flex items-center gap-2 mt-1">
                                    <div className="w-5 h-5 rounded-full bg-accent flex items-center justify-center text-[10px] font-bold text-ink border border-ink shadow-sm">
                                        {commit.author.charAt(0)}
                                    </div>
                                    <span className="text-xs font-bold text-zinc-600">{commit.author}</span>
                                </div>
                            </div>
                        </div>
                    ))}
                    ...
                    {commits.length === 0 && (
                        <div className="text-zinc-400 italic text-sm">No history recorded yet.</div>
                    )}
                </div>
            </div>
        </div>
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div 
                    ...
                    className="fixed top-0 right-0 h-full w-[400px] bg-white border-l-2 border-ink shadow-2xl z-[150] flex flex-col"
                >
